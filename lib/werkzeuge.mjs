...
const frei = {
  bestand_suchen: {
    beschreibung: 'Durchsucht die Bibliothek nach Dateiname, Motiv, Prompt, '
      ...
      + 'nurFavoriten: true liefert die Bilder, die der Mensch als gut '
      + 'markiert hat - sein Urteil, nicht deins. Ihr "motiv" ist der Text, '
      + 'der wirklich funktioniert hat.',
    felder: {
      ...
      nurFavoriten: { type: 'boolean', description: 'nur was als Favorit markiert ist' },
      grenze: { type: 'integer', description: 'hoechstens so viele Treffer, Standard 20' },
    },
    async fuehreAus({
      suche = '', art = null, ordner = null, nurFavoriten = false, grenze = 20,
    }) {
      const { eintraege, hatVideos } = bibliothek.bestandFuerAnsicht({
        suche, art, ordner, nurFavoriten,
      });
