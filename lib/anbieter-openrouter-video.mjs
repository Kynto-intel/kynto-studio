...
//
// Geprueft am 6.9.2026 gegen echte Laeufe: 5 s und 8 s auf kling-v3.0-pro,
// 720p, beide sauber. Der Preis ist streng linear zur Dauer.

import { schluessel } from './konfig.mjs';
import { merkeAufloesungen } from './gelernt.mjs';
...
/**
 * Merkt sich, was ein Modell an Aufloesungen wirklich annimmt.
 *
 * OpenRouter verraet das im Katalog nicht, aber in der Ablehnung: "Supported
 * resolutions: 2K". Anders als beim Bild wird hier NICHT von allein
 * nachgebessert - ein Clip kostet Geld, und in welcher Aufloesung gerendert
 * wird, entscheidet der Mensch. Gemerkt wird es trotzdem, damit die App es
 * beim naechsten Mal VORHER sagen kann, vor dem teuersten Klick der App.
 *
 * Das Gedaechtnis liegt seit 7.9.2026 in daten/gelernt.json - siehe
 * gelernt.mjs. Vorher stand es in einer Map, die jeder Neustart geleert hat,
 * und die Erkenntnis von MiniMax H3 und Seedance 2.5 vom 6.9. war am
 * naechsten Morgen wieder weg.
 */

/** Liest "Supported resolutions: 2K, 4K" aus der Meldung. */
function aufloesungenAus(meldung) {
  const treffer = /supported resolutions?:([^.]+)/i.exec(String(meldung || ''));
  if (!treffer) return [];
  return treffer[1].split(/[,;]/).map((s) => s.trim()).filter(Boolean);
}
...
function deuteFehler(status, text, modell) {
  ...
  // Die haeufigste Ablehnung bei Video: falsche Aufloesung. Die Meldung
  // nennt die erlaubten Werte - also merken und im Klartext sagen, was zu
  // tun ist, statt die englische Rohmeldung durchzureichen.
  const erlaubt = aufloesungenAus(meldung);
  if (erlaubt.length) {
    merkeAufloesungen(modell, erlaubt);
    return `Dieses Modell nimmt die eingestellte Aufloesung nicht. `
      + `Erlaubt ist: ${erlaubt.join(', ')}. Unten in der Leiste umstellen. `
      + `(Der Fehlversuch hat nichts gekostet - abgelehnt wurde vor dem Rendern.)`;
  }
