...
export async function holeBilderNach(aufloesen) {
  for (const v of lade()) {
    ...
    // Aus einem Clip laesst sich mit System.Drawing kein Standbild
    // schneiden - der Versuch scheitert nach 300 ms in einem eigenen
    // PowerShell-Prozess, und zwar bei JEDEM Aufruf dieser Liste. Video
    // zeigt deshalb weiter die Originaldatei; die Oberflaeche kann sie
    // als <video> darstellen.
    if (v.art === 'video') continue;
    try {
