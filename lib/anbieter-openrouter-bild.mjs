...
import { ersatzVerhaeltnis, merkeVerhaeltnis } from './gelernt.mjs';
 ...
 *
 * Gemerkt wird es in daten/gelernt.json und nicht mehr nur im Speicher: bis
 * 7.9.2026 war das Wissen nach jedem Neustart weg, und der erste Lauf des
 * Tages lief zuverlaessig wieder in dieselbe Ablehnung.
 */
...
export async function erzeugeBild({
  ...
  let verhaeltnis = ersatzVerhaeltnis(modell, gewuenscht) || gewuenscht;
  ...
  if (!lauf.ok && lauf.status === 400) {
    ...
    if (ersatz && ersatz !== verhaeltnis) {
      merkeVerhaeltnis(modell, gewuenscht, ersatz, erlaubte);
      verhaeltnis = ersatz;
 ...
 *
 * Mit knapper Frist. Der boese Fall ist nicht "kein Netz" - dann scheitert
 * DNS in einer Sekunde -, sondern "verbunden, aber ohne Internet": Hotel,
 * Cafe, haengender Router. Dann antwortet niemand, und ohne Frist wartet
 * fetch auf die Zeitueberschreitung des Betriebssystems, 20 bis 130
 * Sekunden. Solange saehe der Mensch eine leere Seite, denn /api/start ist
 * das Erste, was die Oberflaeche holt. Vier Sekunden sind langsames WLAN
 * noch allemal wert und ein haengender Anschluss nicht.
 */
