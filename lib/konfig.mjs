...
/**
 * Was man beim Clip einstellen kann.
 *
 * Feste Listen, weil OpenRouter es nicht verraet: nachgesehen am
 * 05.09.2026 nennt keines der 28 Videomodelle in der Modell-Liste, welche
 * Dauern oder Aufloesungen es annimmt, und die Beschreibungen sind
 * abgeschnitten ("Clips range from 3 to..."). Diese Werte sind die, die
 * quer durch Kling, Veo, Runway und Seedance ueblich sind.
 *
 * Nimmt ein Modell einen davon nicht, sagt OpenRouter das in der Ablehnung
 * und nennt die erlaubten - die Meldung wird durchgereicht. Automatisch
 * korrigiert wird hier NICHT, anders als beim Seitenverhaeltnis: dafuer
 * muesste man den Fall einmal echt gesehen haben, und ein Clip kostet zu
 * viel, um ihn zum Ausprobieren zu rendern.
 */
export const VIDEO_DAUERN = [3, 5, 8, 10];
// 2K steht hier, seit Seedance 2.5 und MiniMax H3 am 6.9.2026 mit
// "Resolution 720p is not supported for this model. Supported resolutions:
// 2K" abgelehnt haben. Ohne den Wert in der Liste liess sich der Fehler
// gar nicht beheben - man konnte ihn nur lesen.
export const VIDEO_AUFLOESUNGEN = ['720p', '1080p', '2K'];
...
/**
 * Was die Oberflaeche zuletzt eingestellt hat: Modell und Format.
 *
 * Steht bewusst auf dem Server und nicht nur im Browser. Sonst wuesste
 * niemand ausser dem einen Browserfenster, womit gerendert werden soll -
 * weder der Chat noch ein Programm von aussen. Und
 * genau das ist die Regel: **Das Modell waehlt der Mensch in der App, nicht
 * die KI.** Werkzeuge bekommen deshalb gar kein Modell-Feld, sie erben
 * einfach diese Werte.
 *
 * Die Anzahl steht bewusst NICHT hier - die faengt jedes Mal bei 1 an.
 */
export let STANDARD;

/**
 * Tagesgrenzen in Dollar. `null` heisst: keine Bremse.
 *
 * Bewusst pro Tag und nicht insgesamt: eine Gesamtgrenze waere irgendwann
 * erreicht und die App dann dauerhaft tot, ohne dass jemand merkt warum.
 * Der Tag beginnt nach der Uhr des Rechners neu - siehe kosten.mjs.
 *
 * `gesamt` zaehlt alles zusammen, die anderen drei je Gattung. Beide
 * greifen: wer 5 $ gesamt und 2 $ fuer Video setzt, kommt bei Video nach
 * 2 $ nicht weiter, auch wenn insgesamt noch Luft ist.
 */
export let GRENZEN;
...
function uebernehmen(konfig) {
  ...
  STANDARD = {
    modellBild: s.modellBild || 'google/gemini-3.1-flash-image',
    modellVideo: s.modellVideo || 'kwaivgi/kling-v3.0-std',
    formatId: FORMATE[s.formatId] ? s.formatId : Object.keys(FORMATE)[0],
    chatModell: s.chatModell || 'google/gemini-3.8-flash',
    // Dauer und Aufloesung eines Clips. Stehen hier und nicht nur im
    // Browser, aus demselben Grund wie das Modell: der Assistent und ein
    // Skript ueber die API sollen dasselbe rendern, was in der App steht.
    videoDauer: VIDEO_DAUERN.includes(Number(s.videoDauer)) ? Number(s.videoDauer) : 5,
    videoAufloesung: VIDEO_AUFLOESUNGEN.includes(s.videoAufloesung) ? s.videoAufloesung : '1080p',
  };

  const g = konfig.grenzen || {};
  const betrag = (wert) => {
    const n = Number(wert);
    return Number.isFinite(n) && n > 0 ? Number(n.toFixed(4)) : null;
  };
  GRENZEN = {
    gesamt: betrag(g.gesamt),
    bild: betrag(g.bild),
    video: betrag(g.video),
    chat: betrag(g.chat),
  };
}
...
/**
 * Merkt sich, was zuletzt eingestellt war. Wird bei jeder Aenderung in der
 * Oberflaeche gerufen - klein und oft, deshalb ohne Aufhebens.
 *
 * Unbekannte Formate werden abgelehnt; bei den Modellen wird bewusst NICHT
 * gegen den Katalog geprueft: der laedt sich live von OpenRouter nach, und
 * eine kurze Netzstoerung soll nicht die Einstellung wegwerfen.
 */
export function speichereStandard(neu) {
  const konfig = ladeKonfig();
  const s = { ...STANDARD, ...konfig.standard };

  for (const feld of ['modellBild', 'modellVideo', 'chatModell']) {
    if (typeof neu?.[feld] === 'string' && neu[feld].trim()) s[feld] = neu[feld].trim();
  }
  if (neu?.formatId) {
    if (!FORMATE[neu.formatId]) throw new Error(`Unbekanntes Format: ${neu.formatId}`);
    s.formatId = neu.formatId;
  }

  // Anders als bei den Modellen wird hier gegen die Liste geprueft: sie
  // steht fest im Code und kann nicht durch eine Netzstoerung leer sein.
  if (neu?.videoDauer !== undefined) {
    const d = Number(neu.videoDauer);
    if (!VIDEO_DAUERN.includes(d)) throw new Error(`Unbekannte Clip-Dauer: ${neu.videoDauer}`);
    s.videoDauer = d;
  }
  if (neu?.videoAufloesung !== undefined) {
    if (!VIDEO_AUFLOESUNGEN.includes(neu.videoAufloesung)) {
      throw new Error(`Unbekannte Aufloesung: ${neu.videoAufloesung}`);
    }
    s.videoAufloesung = neu.videoAufloesung;
  }

  konfig.standard = s;
  fs.writeFileSync(KONFIG_DATEI, `${JSON.stringify(konfig, null, 2)}
`, 'utf8');
  STANDARD = s;
  return { ...STANDARD };
}
...
/**
 * Tagesgrenzen speichern. Leer oder 0 heisst: diese Bremse ist aus.
 *
 * Nur die vier bekannten Schluessel werden uebernommen, damit sich in der
 * Datei nichts ansammelt, was niemand mehr zuordnen kann.
 */
export function speichereGrenzen(neu = {}) {
  const konfig = ladeKonfig();
  const alt = konfig.grenzen || {};
  const raus = { ...alt };

  for (const feld of ['gesamt', 'bild', 'video', 'chat']) {
    if (!Object.hasOwn(neu, feld)) continue;
    const wert = neu[feld];
    if (wert === null || wert === '' || wert === undefined) { raus[feld] = null; continue; }
    const n = Number(wert);
    if (!Number.isFinite(n) || n < 0) throw new Error(`Ungueltige Grenze fuer "${feld}": ${wert}`);
    raus[feld] = n > 0 ? Number(n.toFixed(4)) : null;
  }
...
`, 'utf8');
  ...
  // Nur die Grenzen neu setzen, nicht die ganze Konfiguration neu ableiten:
  // Ordner und Formate haben sich nicht geaendert, und was man nicht anfasst,
  // kann man auch nicht kaputtmachen.
  GRENZEN = { ...GRENZEN, ...raus };
  return { ...GRENZEN };
}
