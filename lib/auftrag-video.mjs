...
export async function erzeuge({
  motiv = '', quellBild = null, name = '', mitStil = true,
  ...
  // Der Stil-Block fuer Clips - genau wie beim Bild, nur der andere.
  // Bis 6.9.2026 ging das Motiv hier roh raus; deshalb hatte ein Clip mit
  // demselben Look nie etwas zu tun wie ein Bild aus derselben App.
  const prompt = stil.bauePromptVideo(motiv, mitStil);

  const lauf = await openrouterVideo.erzeugeVideo({
    prompt, modell, breite: m.zielW || m.genW, hoehe: m.zielH || m.genH,
  ...
  sidecar.schreibe(datei, {
    prompt, motiv, anbieter: 'openrouter', modell, format: formatId,
    stilBlock: mitStil ? stil.ladeStilVideo() : '', mitStil,
  ...
  // Sekunden mit, damit die Schaetzung beim naechsten Mal einen Preis
  // nennen kann statt "erst nach dem Lauf bekannt".
  kosten.buche({ clips: 1, dollar: lauf.kosten || 0, modell, sekunden: dauer });
  ...
  verlauf.halteFest({
    ...
    details: {
      motiv, prompt, modell, modellName: modellInfo.name, formatId, dauer, aufloesung,
      mitStil, quellBild: quellBild || null, dollar: lauf.kosten, sekunden: lauf.dauer,
