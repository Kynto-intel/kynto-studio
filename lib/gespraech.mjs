...
export function systemHinweis(siehtBilder = false) {
  ...
  return [
    ...
    '    -> bestand_suchen',
    '- Frage nach Kosten, Preis, Modell, Format, Tagesgrenze - was ein Lauf',
    '  JETZT kosten wuerde',
    '    -> einstellung_lesen',
    // Die Trennlinie zwischen den beiden: bestand_suchen sagt, WAS da liegt,
    // verlauf_lesen sagt, WAS PASSIERT IST. Ohne diesen Unterschied im Text
    // beantwortet ein Modell "was habe ich gestern gemacht" mit einer
    // Dateiliste - richtig sortiert, aber die falsche Frage.
    '- Frage nach FRUEHER: "was habe ich gestern gemacht", "was hat der letzte',
    '  Clip gekostet", "welchen Prompt hatte ich heute frueh"',
    '    -> verlauf_lesen. Das ist die Geschichte, nicht der Bestand',
    '- Frage nach dem AUSGEGEBENEN Geld: "was habe ich diesen Monat',
    '  ausgegeben", "wofuer ging das Geld drauf", Schnittkosten je Bild',
    '    -> kosten_lesen. Nicht einstellung_lesen, das sagt nur, was ein',
    '       Lauf JETZT kosten wuerde',
    '- Mockup, Spruchbild, Post - etwas, das er oefter so macht',
    '    -> vorlage_lesen, BEVOR du einen Prompt erfindest',
    // Ein Favorit ist billiger zu setzen als eine Vorlage und es gibt
    // deshalb viel mehr davon. Als Rohstoff fuer "mehr in der Art" ist er
    // die bessere Quelle - nur schwaecher gemeint, also nie eins zu eins
    // uebernehmen.
    '- "mehr in der Art", "so wie das letzte gute", "aehnlich aber anders"',
    '    -> bestand_suchen mit nurFavoriten. Das sind die Bilder, die IHM',
    '       gefallen haben. Nimm den AUFBAU ihrer Motive - Bildausschnitt,',
    '       Licht, Haltung, Satzbau - und tausche das Motiv aus. Schreib den',
    '       Text NICHT ab: er will etwas Neues in derselben Handschrift,',
    '       nicht dasselbe Bild noch einmal.',
    '- Er will eine Vorlage umschreiben lassen',
    '    -> prompt_vorschlagen, dann steht dein Text in seinem Feld',
    ...
    // Diese Zeile ist konkreter als der allgemeine Anleitungs-Verweis
    // weiter unten und feuert deshalb zuerst - gemessen am 9.9.2026, das
    // Modell war zufrieden und hat die Anleitung nie gelesen. Also muss
    // der Verweis HIER stehen, nicht nur allgemein.
    '- Spruch oder Text soll ins Bild',
    '    -> ZUERST anleitung_lesen "text-im-bild", dann text_aufs_bild',
    '- Favorit, Freigabe oder Bildunterschrift setzen',
    '    -> datei_markieren',
    '- Neues Bild oder Clip gewuenscht',
    '    -> bild_erzeugen / video_erzeugen als VORSCHLAG',
    // Die Anleitung kommt VOR dem Prompt, nicht danach. Ein Modell, das
    // erst schreibt und dann nachliest, hat den Prompt schon verdorben.
    '- Die Aufgabe passt zu einer Anleitung aus der Liste unten',
    '    -> anleitung_lesen ZUERST, dann den Prompt bauen',
    '',
    ...
    '',
    ...ansehen,
    // Nur Name und Ausloeser je Anleitung, nicht ihr Inhalt. Der volle
    // Text kommt ueber anleitung_lesen und nur, wenn er gebraucht wird.
    ...anleitungen.fuerHinweis(),
