...
| `anleitungen.mjs` | Handwerk fuer EINE Aufgabe, aus `daten/anleitungen/*.md`. Die vierte Textschicht neben Stil-Block, Regie und Vorlagen: die Vorlage wiederholt EINEN Prompt, die Anleitung sagt, wie man diese ART Prompt baut. In den Systemhinweis geht nur die LISTE (Name plus "Wann"-Zeile), der volle Text erst ueber `anleitung_lesen` - sonst zahlte man bei jedem Zug fuer alle. Der Assistent liest sie, schreibt sie nie |
| `gelernt.mjs` | was die App aus Ablehnungen gelernt hat, in `daten/gelernt.json`: welches Seitenverhaeltnis ein Bildmodell statt des gewuenschten nimmt, welche Aufloesungen ein Videomodell annimmt. Eingetragen wird NUR, was ein Anbieter woertlich abgelehnt hat — nie eine Vermutung. Wird einmal gelesen und dann im Speicher gehalten; wer die Datei von Hand aendert, muss die App neu starten |
